...
/* the tables of nodes and edges are statically allocated. Modify the 
   ...
   necessary */
#define tab_node_size  1000
#define tab_edge_size 400000
#define max_nb_value 200
...
int main(int argc, char *argv[]) {
  ...
  case TRUE:
	...
	printf("%d-Colorable, Solution verified\n", NB_VALUE);
	print_solution(filename(INPUT_FILE), NB_VALUE);
      }
      else printf("Solution wrong\n");
    }
    else printf("Not %d-colorable\n", NB_VALUE);
    break;
  }
  mess=times(a_tms); endtime = a_tms->tms_utime;

  printf("NB_UNIT= %d, NB_BACK= %d, NB_BRANCHE= %d\n", NB_UNIT, NB_BACK, NB_BRANCHE);
  ...
  fprintf(fp_time, "color6 %s %5.3f %d %d %d %d %d %d %d %d\n", 
	  INPUT_FILE, ((double)(endtime-begintime)/CLOCKS_PER_SEC), 
